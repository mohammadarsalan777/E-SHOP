...
const Search = ({ products, GridList }) => {
    ...
    // const filterProducts = productsfilter((product) => product.name.toLowerCase().includes(searchTerm.toLowerCase()))
    const filterProducts = products.filter((product) => product.name.toLowerCase().includes(searchTerm.toLowerCase()))

    return (
        <div className='widget widget-search'>
            <form className="search-wrapper mb-3">
                <input type="text" name="search" id="search" placeholder='Search product' defaultValue={searchTerm}
